...
Here, we’ll use Streamlit to look at some of the state-of-the-art LLMs on Hugging Face that can easily be set-up for quick inference in a chatbot interface:
...
[Gemma](https://blog.google/technology/developers/gemma-open-models/) is described as a family of lightweight models. It was developed by Deepmind and other teams at Google. These models were used to create Google’s Gemini models. The name of the model comes from the Latin word *gemma*, translating to 'gem' or precious stone.
...
One thing to note is that the current 7B Hugging Face implementation of the Gemma model may have some hiccups, which are looked into by the Hugging Face team[^6] so use it with caution.
...
5.  We configure the session settings in Streamlit and add some buttons.
...
6.  We do some configurations to clean up our messages and make sure we have the model selected.
...
7.  Finally we set up the chat interface in Streamlit where the user’s questions or `prompts` get passed on to the model we select and the model or the `assistant` returns a `response`. We make sure to keep adding our `prompts` and `response`s to the *messages* structure we created so the model has some context to keep a conversation going.
